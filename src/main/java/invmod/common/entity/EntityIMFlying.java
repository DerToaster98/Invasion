...
public abstract class EntityIMFlying extends EntityIMLiving
{
    ...
    protected float calcBlockPathCost(PathNode prevNode, PathNode node, IBlockAccess terrainMap)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            for (int j = 1; j <= 2; j++)
            {
                int id = terrainMap.getBlockId(node.xCoord + invmod.common.util.CoordsInt.offsetAdjX[i] * j, node.yCoord, node.zCoord + invmod.common.util.CoordsInt.offsetAdjZ[i] * j);
                int blockType = getBlockType(id);
        ...
        {
            return prevNode.distanceTo(node) * ((Float) EntityIMLiving.blockCosts.get(Integer.valueOf(id))).floatValue() * multiplier;
        }
