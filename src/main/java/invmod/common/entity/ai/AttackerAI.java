...
public class AttackerAI
{
    ...
    public void readFromNBT(NBTTagCompound nbttagcompound)
    {
        ...
        {
            Scaffold scaffold = new Scaffold(this.nexus);
            scaffold.readFromNBT((NBTTagCompound) nbtScaffoldList.tagAt(i));
    ...
    private void orientScaffold(Scaffold scaffold, IBlockAccess terrainMap)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                if (terrainMap.isBlockNormalCube(scaffold.getXCoord() + invmod.common.util.CoordsInt.offsetAdjX[i], scaffold.getYCoord() + height, scaffold.getZCoord() + invmod.common.util.CoordsInt.offsetAdjZ[i]))
                {
                    blockCount++;
                }

                if (terrainMap.isBlockNormalCube(scaffold.getXCoord() + invmod.common.util.CoordsInt.offsetAdjX[i] * 2, scaffold.getYCoord() + height, scaffold.getZCoord() + invmod.common.util.CoordsInt.offsetAdjZ[i] * 2))
                {
                    blockCount++;
                }
            }
