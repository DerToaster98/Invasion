...
public class EntityAIBirdFight<T extends EntityLivingBase> extends EntityAIMeleeFight<T>
{
    ...
    {
        int knockback = 2;
        target.addVelocity(-MathHelper.sin(this.theEntity.rotationYaw * (float)Math.PI / 180.0F) * knockback * 0.5F, 0.4D, MathHelper.cos(this.theEntity.rotationYaw * (float)Math.PI / 180.0F) * knockback * 0.5F);
