...
public class NavigatorBurrower extends NavigatorParametric
{
    ...
    private PosRotate3D calcPositionAndRotation(float time, PathNode start, PathNode middle, PathNode end)
    {
        ...
        if ((hZ == 1) && (gZ == 1))
        {
            ...
            posY = 0.5D;
            posX = 0.5D;
            rotY = vZ * (float)Math.PI / 4.0F;
            return new PosRotate3D(posX, posY, posZ, rotX, rotY, rotZ);
        }

        if (hX == 1)
        {
            posX = vX * hX * Math.sin(time * 0.5D * Math.PI) * 0.5D + xOffset;
        }
        else
        {
            posX = vX * hX * Math.cos(time * 0.5D * Math.PI) * 0.5D + xOffset;
        }

        if (hY == 1)
        {
            posY = vY * hY * Math.sin(time * 0.5D * Math.PI) * 0.5D + yOffset;
        }
        else
        {
            posY = vY * hY * Math.cos(time * 0.5D * Math.PI) * 0.5D + yOffset;
        }

        if (hZ == 1)
        {
            posZ = vZ * hZ * Math.sin(time * 0.5D * Math.PI) * 0.5D + zOffset;
        }
        else
        {
            posZ = vZ * hZ * Math.cos(time * 0.5D * Math.PI) * 0.5D + zOffset;
        }
        ...
        posY += 0.5D;
        posZ += 0.5D;
        rotX /= (180F / (float)Math.PI);
        rotY /= (180F / (float)Math.PI);
        rotZ /= (180F / (float)Math.PI);
        return new PosRotate3D(posX, posY, posZ, rotX, rotY, rotZ);
    }
