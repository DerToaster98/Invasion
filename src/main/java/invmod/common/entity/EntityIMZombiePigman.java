...
public class EntityIMZombiePigman extends EntityIMMob implements ICanDig 
{
	...
	{
		if ((this.isImmuneToFire) && ((block == Block.fire.blockID) || (block == Block.lavaMoving.blockID) || (block == Block.lavaStill.blockID) )) 
		{
			return false;
		}
	...
	{
		Integer block = this.worldObj.getBlockId(x, y, z);
		return (block == 0) || (isBlockDestructible(this.worldObj, x, y, z, block));
    ...
    public boolean isBlockDestructible(IBlockAccess terrainMap, int x, int y, int z, int id)
    {
        ...
        return (!isTooSteep) && (super.isBlockDestructible(terrainMap, x, y, z, id));
    }
	      ...
	      if(!worldObj.isRemote)
	      {
	        for(int i=y;i<=y+1;i++)
	        {
	        	for(int j=x-1;j<=x+1;j++)
	        	{
	        		...
	        		{
	        			int block=worldObj.getBlockId(j, i, k);
	        		    int meta = worldObj.getBlockMetadata(j,i,k);

	        			if(block!=0){
	        				if(isBlockDestructible(this.worldObj, j, i, k,block) && block!=mod_Invasion.blockNexus.blockID)
	        				{
	        				this.playSound("random.explode", 0.2F, 0.5F);	        		      	
	        		    	  if(mod_Invasion.getDestructedBlocksDrop())
	        		    	  {
	        		    		  Block.blocksList[block].dropBlockAsItem(this.worldObj, j, i, k, meta,0);
	        		    	  }
	        				worldObj.setBlock(j,i,k, 0);
	        				}
	        			}
	        		}
	        	}
    ...
    protected void updateSound()
    {
        if (this.terrainModifier.isBusy())
        {
            ...
            {
                this.worldObj.playSoundAtEntity(this, "invmod:scrape" + (rand.nextInt(2) + (Integer)1), 0.85F, 1.0F / (this.rand.nextFloat() * 0.5F + 1.0F));
	...
	private void setAttributes(int tier, int flavour) {
		...
		if (tier == 1) {
			...
			if(this.rand.nextInt(5)==1){
				this.setCurrentItemOrArmor(2, new ItemStack(Item.plateGold,1));
			}
			
			if(this.rand.nextInt(5)==1){
				this.setCurrentItemOrArmor(3, new ItemStack(Item.legsGold,1));
			}
			
			if(this.rand.nextInt(5)==1){
				this.setCurrentItemOrArmor(4, new ItemStack(Item.bootsGold,1));
			}
