...
public class NavigatorFlying extends NavigatorIM implements INavigationFlying
{
    ...
    protected Vec3 convertToVector(float yaw, float pitch, float idealSpeed)
    {
        ...
        double x = this.theEntity.posX + -Math.sin(yaw / 180.0F * Math.PI) * idealSpeed * time;
        double y = this.theEntity.posY + Math.sin(pitch / 180.0F * Math.PI) * idealSpeed * time;
        double z = this.theEntity.posZ + Math.cos(yaw / 180.0F * Math.PI) * idealSpeed * time;
    ...
    protected void updateHeading()
    {
        ...
        for (int i = 0; i < 30; i++)
        {
            ...
            for (int j = 0; j < 20; j++)
            {
                ...
                double y = this.theEntity.posY + Math.sin(nextAngleV / 180.0D * Math.PI) * this.visionDistance;
                double distanceXZ = Math.cos(nextAngleV / 180.0D * Math.PI) * this.visionDistance;
                double x = this.theEntity.posX + -Math.sin(nextAngleH / 180.0D * Math.PI) * distanceXZ;
                double z = this.theEntity.posZ + Math.cos(nextAngleH / 180.0D * Math.PI) * distanceXZ;
                Vec3 target = this.theEntity.worldObj.getWorldVec3Pool().getVecFromPool(x, y, z);
                Vec3 origin = this.theEntity.getPosition(1.0F);
                origin.yCoord += 1.0D;
                MovingObjectPosition object = this.theEntity.worldObj.clip(origin, target);
    ...
    private Pair<Float, Float> appraiseLanding()
    {
        ...
        for (int i = 0; i < landingResolution; i++)
        {
            ...
            double y = this.theEntity.posY + Math.sin(nextAngleV / 180.0D * Math.PI) * 64.0D;
            ...
            if (object != null)
            {
                int id = this.theEntity.worldObj.getBlockId(object.blockX, object.blockY, object.blockZ);

                if (!this.theEntity.avoidsBlock(id))
                {
                    safety += 0.7F;
                }
