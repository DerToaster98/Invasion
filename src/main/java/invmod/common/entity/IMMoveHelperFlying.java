...
public class IMMoveHelperFlying extends IMMoveHelper
{
    ...
    {
        double x = this.a.posX + Math.sin(yaw / 180.0F * Math.PI) * idealSpeed * time;
        double y = this.a.posY + Math.sin(pitch / 180.0F * Math.PI) * idealSpeed * time;
        double z = this.a.posZ + Math.cos(yaw / 180.0F * Math.PI) * idealSpeed * time;
    ...
    protected FlyState fly()
    {
        ...
        if (distanceSquared > 0.04D)
        {
            ...
            else
            {
                ...
                double desiredXZHeading = Math.atan2(dZ, dX) - (Math.PI / 2D);
                double currXZHeading = Math.atan2(velZ, velX) - (Math.PI / 2D);
                double dXZHeading = desiredXZHeading - currXZHeading;

                while (dXZHeading >= Math.PI)
                {
                    dXZHeading -= (Math.PI * 2D);
                }
                ...
                if (velY - gravity < 0.0D)
                {
                    ...
                    if (xzSpeed > 0.0D)
                    {
                        velPitch = Math.atan(velY / xzSpeed) / Math.PI * 180.0D;
                    }
                    else
                    {
                        velPitch = -180.0D;
    ...
    protected FlyState doLanding()
    {
        ...
        for (int i = 1; i < 5; i++)
        {
            if (this.a.worldObj.getBlockId(x, y - i, z) != 0)
            {
                break;
            }
    ...
    protected Vec3 calcThrust(double desiredVThrustRatio)
    {
        ...
        double xAccel = hThrust * -Math.sin(this.a.rotationYaw / 180.0F * Math.PI);
        double yAccel = vThrust;
        double zAccel = hThrust * Math.cos(this.a.rotationYaw / 180.0F * Math.PI);
        return this.a.worldObj.getWorldVec3Pool().getVecFromPool(xAccel, yAccel, zAccel);
    }
}