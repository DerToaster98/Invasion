...
public class ModelBird extends ModelBase
{
    ...
    public void setFlyingAnimations(float flapProgress, float legSweepProgress, float roll)
    {
        ...
        this.body.rotateAngleZ = (-roll / 180.0F * (float)Math.PI);
        ...
        this.body.rotationPointY = (7.0F + MathHelper.cos(flapProgress * (float)Math.PI * 2.0F) * 1.4F);
        ModelRenderer tmp190_187 = this.thighR;
        tmp190_187.rotateAngleX = ((float)(tmp190_187.rotateAngleX + MathHelper.cos(flapProgress * (float)Math.PI * 2.0F) * 0.08726646324990228D));
        ModelRenderer tmp218_215 = this.thighL;
        tmp218_215.rotateAngleX = ((float)(tmp218_215.rotateAngleX + MathHelper.cos(flapProgress * (float)Math.PI * 2.0F) * 0.08726646324990228D));
        this.tail.rotateAngleX = ((float)(0.2617993956013792D + MathHelper.cos(flapProgress * (float)Math.PI * 2.0F) * 0.03490658588512815D));
        this.head.rotateAngleX = ((float)(-(Math.PI / 10D) - MathHelper.cos(flapProgress * (float)Math.PI * 2.0F) * 0.03490658588512815D));
    }
    ...
    public void setRotationAngles(float limbPeriod, float limbMaxMovement, float ticksExisted, float headYaw, float entityPitch, float unitScale, Entity entity)
    {
        ...
        this.body.rotateAngleX = (((float)Math.PI / 2F) - entityPitch / 180.0F * (float)Math.PI);
    }
}