...
public class ModelVulture extends ModelBase
{
    ...
    public void setFlyingAnimations(AnimationState wingState, AnimationState legState, AnimationState beakState, float roll, float headYaw, float headPitch, float parTick)
    {
        ...
        if (wingAction == AnimationAction.WINGFLAP)
        {
            ...
            this.body.rotationPointY += MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 1.4F;
            ModelRenderer tmp244_241 = this.rightThigh;
            tmp244_241.rotateAngleX = ((float)(tmp244_241.rotateAngleX + MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 0.08726646324990228D));
            ModelRenderer tmp274_271 = this.leftThigh;
            tmp274_271.rotateAngleX = ((float)(tmp274_271.rotateAngleX + MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 0.08726646324990228D));
            ModelRenderer tmp304_301 = this.tail;
            tmp304_301.rotateAngleX = ((float)(tmp304_301.rotateAngleX + MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 0.03490658588512815D));
        }
    ...
    public void setRotationAngles(float limbPeriod, float limbMaxMovement, float ticksExisted, float headYaw, float entityPitch, float unitScale, Entity entity)
    {
        ...
        tmp19_16.rotateAngleX = ((float)(tmp19_16.rotateAngleX + (0.8707963705062867D - entityPitch / 180.0F * (float)Math.PI)));
