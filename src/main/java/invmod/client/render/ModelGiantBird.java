...
public class ModelGiantBird extends ModelBase
{
    ...
    public void setFlyingAnimations(AnimationState wingState, AnimationState legState, float roll, float headYaw, float headPitch, float parTick)
    {
        ...
        if (wingAction == AnimationAction.WINGFLAP)
        {
            ...
            tmp208_205.rotateAngleX = ((float)(tmp208_205.rotateAngleX + MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 0.08726646324990228D));
            ModelRenderer tmp238_235 = this.leftThigh;
            tmp238_235.rotateAngleX = ((float)(tmp238_235.rotateAngleX + MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 0.08726646324990228D));
            ModelRenderer tmp268_265 = this.tail;
            tmp268_265.rotateAngleX = ((float)(tmp268_265.rotateAngleX + MathHelper.cos(flapCycle * (float)Math.PI * 2.0F) * 0.03490658588512815D));
        }
